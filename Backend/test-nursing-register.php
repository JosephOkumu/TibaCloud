<?php
// Test script for nursing facility registration

// Display script execution start
echo "Starting test for nursing facility registration...\n";

// API endpoint - make sure there's no leading slash in the path part
$url = 'http://127.0.0.1:8080/api/register';
echo "Target URL: $url\n";

// Nursing facility registration data
$data = [
    'name' => 'Test Nursing Facility',
    'email' => 'testnursing'.time().'@example.com', // Using timestamp to ensure unique email
    'password' => 'password123',
    'password_confirmation' => 'password123',
    'phone_number' => '0700456789',
    'user_type' => 'nursing',
    'license_number' => 'NUR'.rand(1000, 9999).'/'.date('Y'),
...
if ($statusCode === 201 && isset($responseData['access_token'])) {
    echo "SUCCESS: Nursing facility registration was successful!" . PHP_EOL;
    echo "User ID: " . $responseData['user']['id'] . PHP_EOL;
    echo "User Type: " . $responseData['user']['user_type'] . PHP_EOL;
} else {
    echo "ERROR: Nursing facility registration failed!" . PHP_EOL;
