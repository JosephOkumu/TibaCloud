...
echo "Starting test for patient registration...\n";

// API endpoint - make sure there's no leading slash in the path part
$url = 'http://127.0.0.1:8080/api/register';
echo "Target URL: $url\n";

// Patient registration data (based on the image)
$data = [
    'name' => 'Test Patient',
    'email' => 'testpatient'.time().'@example.com', // Using timestamp to ensure unique email
    'password' => 'password123',
    'password_confirmation' => 'password123',
    'phone_number' => '0700111222',
    'user_type' => 'patient'
    // Note: Patient doesn't need license_number or national_id
];
...
if ($statusCode === 201 && isset($responseData['access_token'])) {
    echo "SUCCESS: Patient registration was successful!" . PHP_EOL;
    echo "User ID: " . $responseData['user']['id'] . PHP_EOL;
    echo "User Type: " . $responseData['user']['user_type'] . PHP_EOL;
} else {
    echo "ERROR: Patient registration failed!" . PHP_EOL;
